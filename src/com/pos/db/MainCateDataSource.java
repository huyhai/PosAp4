...
public class MainCateDataSource {
	...
	public MainCateDataSource(Context c) {
	...
	public String checkgroupid(String maVatTu) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "select GroupID from group_translations where Name = '" + maVatTu + "' LIMIT 1";
	...
	public String checkgroupcode(String maVatTu) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "select Group_Code from Group_item where GroupID = '" + maVatTu + "' LIMIT 1";									
				c = dbHelper.getDb().rawQuery(query, null);
	...
	public String checkgroupCN(String maVatTu) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "select Name from group_translations where GroupID = '" + maVatTu + "' AND LanguageID=2 LIMIT 1";									
				c = dbHelper.getDb().rawQuery(query, null);
	...
	public void update(String image,String groupcode,String groupid,String name,String nameCN,String ts) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
                ...
                cv.put("TextSize", ts);
                dbHelper.getDb().update(MAINCATE_TABLE_NAME, cv, "GroupID="+groupid, null);
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				ContentValues cv = new ContentValues();
				cv.put("Name", name);      
                dbHelper.getDb().update("group_translations", cv, "GroupID="+groupid+" AND LanguageID=1", null);
	...
	public void update2(String image,String groupcode,String groupid,String name,String nameCN) {
		
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				ContentValues cv = new ContentValues();
				cv.put("Name", nameCN);      
                dbHelper.getDb().update("group_translations", cv, "GroupID="+groupid+" AND LanguageID=2", null);
	...
	public String DelelteMainCate(String id) {
		...
		try
		{
				if (dbHelper != null && dbHelper.getDb() != null) {
					...
					int i=dbHelper.getWritableDatabase().update("Group_item", newValues, "GroupID="+id, null);
					error+=i;
					dbHelper.close();
	...
	public ArrayList<MainCateModel> loadMainCate() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "SELECT Group_item .*, group_translations.Name,group_translations.Description FROM Group_item  INNER JOIN group_translations ON Group_item.GroupID=group_translations.GroupID WHERE group_translations.LanguageID=1 AND Active=1";
