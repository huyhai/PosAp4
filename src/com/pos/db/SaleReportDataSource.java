...
return query+s;
    ...
    public String loadOpenCashd(String form,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Cash_float_start from counters where Date between '"+form+"' AND '"+to+"' order by Date DESC LIMIT 1";
    ...
    public String loadGrossSale() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";
    ...
    public String loadGrossSaled(String from,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where Sales_date between '"+from+"' AND '"+to+"'";
    ...
    public String loadItemSale() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select sale_details.Amount from sale_details inner join sales on sales.SaleID=sale_details.SaleID where sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadItemSaled(String form,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select sale_details.Amount from sale_details inner join sales on sales.SaleID=sale_details.SaleID where sales.Sales_date between '"+form+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadUserName2(String usserID) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select UserName from users WHERE IDUser="+usserID;										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadBillDisccount() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Discount_amount from sales where Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadStockind(String form,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select StockInHand from Inventory_report where Date between '"+form+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadAud(String form,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Remarks,Cost_Price from items where Date between '"+form+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadstockvalue() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select CostPrice,StockInHand,TotalIn,TotalOut from Inventory_report";		
//			String query = "Select StockInHand,TotalIn,Selling_price_1 from Inventory_report INNER JOIN items WHERE items .Item_code=Inventory_report.ItemCode";		
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
						try {							
							Double costprice=Double.parseDouble(c.getString(0));
							Double stockinhand=Double.parseDouble(c.getString(1));
							Double totalin=Double.parseDouble(c.getString(2));
							Double totalout=Double.parseDouble(c.getString(3));
							Log.v("HAI", costprice+""+stockinhand+totalin+""+totalout);
//							Double temp3=Double.parseDouble(c.getString(2));
//							temp=(temp1+temp2)*temp3;
							temp=((stockinhand+totalin)-totalout)*costprice;;
							i+=temp;
						} catch (Exception e) {
						}
					} while (c.moveToNext());
...
//		s+=df.format(i);		
    ...
    public String loadstockvalued(String form,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select StockInHand,TotalIn,Selling_price_1 from Inventory_report INNER JOIN items WHERE items .Item_code=Inventory_report.ItemCode AND Inventory_report.Date between '"+form+"' AND '"+to+"'";		
//				String query = "Select Remarks,Selling_price_1 from items where Date between '"+form+"' AND '"+to+"'";	
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
...
//		s+=df.format(i);		
    ...
    public String loadStockin2d(String form,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select TotalIn from Inventory_report where Date between '"+form+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
...
//				String query = "Select TotalOut from Inventory_report ";
    ...
    public String loadStockoutd(String form,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select TotalOut from Inventory_report where Date between '"+form+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadreturnd(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadBillDisccountd(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Discount_amount from sales where Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadItemDisccount() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Discount from sale_details inner join sales on sales.SaleID=sale_details.SaleID where sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadItemDisccountd(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Discount from sale_details inner join sales on sales.SaleID=sale_details.SaleID where sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadNumberOfSale() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select * from sales where Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadNumberOfSaled(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select * from sales where Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadGST() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				query = "Select GST from sales where Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
						try {
							Double tempGST=Double.parseDouble(c.getString(0).toString());
							temp+=tempGST;
    ...
    public String loadItemSaleShift() {
		String s="";
		Cursor c = null;
        Double temp=0.0;
        String sdate=loadShiftDate();
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select ItemName,sum(Quantity) from sale_details inner join items on sale_details.ItemCode=items.Item_code inner join sales on sales.SaleID=sale_details.SaleID where Sales_date between '"+sdate+"' AND '"+currentDateandTime+"' group by ItemName ";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadItemIName() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select ItemName,Sum(Quantity) as qty from sale_details inner join items on sale_details.ItemCode=items.Item_code inner join sales on sales.SaleID=sale_details.SaleID group by ItemName order by qty DESC LIMIT 10";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadItemIqty() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Sum(Quantity) as qty from sale_details inner join items on sale_details.ItemCode=items.Item_code inner join sales on sales.SaleID=sale_details.SaleID group by ItemName order by qty DESC LIMIT 10";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadUnPaid() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount,Status from HoldSales where Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadUnPaidd(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from HoldSales where Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSale() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where payment_mode=2 AND sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Type1,Type1Amount from sale_payments inner join sales on sales.SaleID=sale_payments.SaleID where sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
						try {
							if (!c.getString(0).equals("CASH")&&!c.getString(0).equals(""))
							{
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Type2,Type2Amount from sale_payments inner join sales on sales.SaleID=sale_payments.SaleID where sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSaled(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where payment_mode=2 AND sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSaledMaster1(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where CreditType='MASTER' AND payment_mode=2 AND sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSaledNETS(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where CreditType='NETS' AND payment_mode=2 AND sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSaledVISA(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where CreditType='VISA' AND payment_mode=2 AND sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSaledAMEX(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where CreditType='AMEX' AND payment_mode=2 AND sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSaledDINERS(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where CreditType='DINERS' AND payment_mode=2 AND sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadCardSaledJCB(String fo,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where CreditType='JCB' AND payment_mode=2 AND sales.Sales_date between '"+fo+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadTotalCash() {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where payment_mode=1 AND sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Change,Type1Amount,Type1 from sale_payments inner join sales on sales.SaleID=sale_payments.SaleID where Type1='CASH' AND sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Change,Type2Amount,Type2 from sale_payments inner join sales on sales.SaleID=sale_payments.SaleID where Type2='CASH' AND sales.Sales_date between '"+sdate+"' AND '"+currentDateandTime+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
    ...
    public String loadTotalCashd(String fr,String to) {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Total_amount from sales where payment_mode=1 AND sales.Sales_date between '"+fr+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Change,Type1Amount,Type2 from sale_payments inner join sales on sales.SaleID=sale_payments.SaleID where Type1='CASH' AND sales.Sales_date between '"+fr+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
		...
		try {
			if (dbHelper != null && dbHelper.getDb() != null) {
				String query = "Select Change,Type2Amount,Type2 from sale_payments inner join sales on sales.SaleID=sale_payments.SaleID where Type2='CASH' AND sales.Sales_date between '"+fr+"' AND '"+to+"'";										
				c = dbHelper.getDb().rawQuery(query, null);
				if (c != null && c.getCount() > 0) {					
					c.moveToFirst();
					do {
